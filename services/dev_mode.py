...
使用者可透過特殊指令在 customer / driver / boss 之間切換，
...
VALID_ROLES = {'customer', 'driver', 'boss'}
...
def handle_dev_command(user_id: str, user_msg: str) -> str | None:
    ...
    if msg.startswith('/as '):
        ...
        if len(parts) == 2 and parts[1] in VALID_ROLES:
            ...
            logger.info(f"[DEV] User {user_id} switched to role: {role}")
            return (
                f"[DEV] 已切換為「{role}」模式。\n"
                f"現在傳送任何訊息都會以 {role} 身份處理。\n"
                f"輸入 /whoami 確認，輸入 /as customer|driver|boss 切換角色。"
            )
        else:
            return f"[DEV] 用法：/as customer | /as driver | /as boss"
