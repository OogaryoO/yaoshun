...
_customer_next_input_mode: dict[str, str] = {}
...
def _handle_boss_message(event: MessageEvent, line_bot_api: LineBotApi, user_msg: str, user_id: str):
    ...
    if user_id in _boss_next_input_mode and _starts_with_known_command(user_msg, _BOSS_COMMAND_PREFIXES) and not user_msg.startswith("查詢客戶"):
        _boss_next_input_mode.pop(user_id, None)
        _boss_pending_data.pop(user_id, None)

    if user_id in _boss_next_input_mode and not _starts_with_known_command(user_msg, _BOSS_COMMAND_PREFIXES):
        mode = _boss_next_input_mode.pop(user_id)
        data = _boss_pending_data.pop(user_id, {})
        if mode == "customer_search":
        ...
        elif mode == "unpaid_range":
            range_keyword = user_msg.strip()
            ...
            if len(parts) < 3 or not parts[-1].isdigit() or not parts[-2].isdigit():
                ...
                reply_text = "請輸入 客戶姓名 訂單編號 數量，例：王小明 3 2"
                LineService.reply_text(line_bot_api, event.reply_token, reply_text)
                return

            quantity = int(parts[-1])
            idx = int(parts[-2])
            customer_name = " ".join(parts[:-2]).strip()
            if not customer_name:
                _boss_next_input_mode[user_id] = "boss_help_order"
                _boss_pending_data[user_id] = {}
                reply_text = "請輸入客戶姓名，例：王小明 3 2"
                LineService.reply_text(line_bot_api, event.reply_token, reply_text)
                return
            ...
            if idx < 1 or idx > len(products):
                _boss_next_input_mode[user_id] = "boss_help_order"
                _boss_pending_data[user_id] = {}
                reply_text = f"商品編號 {idx} 不存在，請輸入正確編號。"
                LineService.reply_text(line_bot_api, event.reply_token, reply_text)
                return
            ...
            if len(customers) > 1:
                exact = [c for c in customers if c.get("displayName", "") == customer_name]
                if len(exact) == 1:
                    customer = exact[0]
                else:
                    _boss_next_input_mode[user_id] = "boss_help_order"
                    _boss_pending_data[user_id] = {"idx": idx, "quantity": quantity}
                    lines = [f"找到多位符合「{customer_name}」的客戶，請輸入更完整的姓名："]
                    for c in customers:
                        lines.append(f"• {c.get('displayName', '未知客戶')}")
                    reply_text = "\n".join(lines)
                    LineService.reply_text(line_bot_api, event.reply_token, reply_text)
                    return
            else:
            ...
            customer_id = customer.get("userId", "") if customer else f"{_MANUAL_CUSTOMER_PREFIX}{customer_name}"
            try:
                ...
                items_str = f"{product.get('productName', '')} x{quantity}"
                LineService.reply_flex(
                    line_bot_api,
                    event.reply_token,
                    "訂單建立成功",
                    _flex_boss_order_success(order_id, resolved_name, items_str, unit_price * quantity, "")
                )
                return
            except Exception as e:
            ...
            if len(parts) == 1 and parts[0].isdigit():
                ...
                return
            if quantity <= 0:
                reply_text = "數量請輸入正整數，例：2"
                LineService.reply_text(line_bot_api, event.reply_token, reply_text)
                return
            try:
                products = FirebaseDB.get_products()
                if idx is None or idx < 1 or idx > len(products):
                    reply_text = "商品編號不存在，請輸入有效編號。"
                    LineService.reply_text(line_bot_api, event.reply_token, reply_text)
                    return
                product = products[idx - 1]
                unit_price = _product_unit_price(product)
            ...
            if len(customers) > 1:
                ...
                return
            customer = customers[0] if customers else None
            data_pending = data or {}
            product = data_pending.get("product") or _boss_pending_data.get(user_id, {}).get("product")
            idx = data_pending.get("product_idx") or _boss_pending_data.get(user_id, {}).get("product_idx")
            quantity = data_pending.get("quantity") or _boss_pending_data.get(user_id, {}).get("quantity")
            resolved_name = customer.get("displayName", customer_name) if customer else customer_name
            customer_id = customer.get("userId", "") if customer else ""
            # 建立真正的 pending orders，下一則非指令訊息視為地址
            ...
            LineService.reply_flex(
                ...
                _flex_boss_address_prompt(
                    resolved_name,
                    _product_summary_text(product.get("productName", ""), quantity, _product_unit_price(product)),
                ),
            )
            return
            ...
            if len(parts) != 2:
                reply_text = (
                    ...
                    "例：ORD-20260514-101530-a1b2 cash"
                )
            else:
        ...
        elif mode == "mark_delivered":
            order_id = user_msg.strip()
            if not order_id:
    ...
    if user_msg == "取消下單":
        ...
        # 若先前有發起「幫客戶下單」流程，進入幫客戶下單模式：要求輸入數量
    ...
    # ── 預設提示 ──────────────────────────────────────────────────────
        ...
        reply_text = (
            ...
            "• 點擊「查詢客戶」，輸入：關鍵字\n"
            "• 點擊「客戶訂單」，輸入：姓名\n"
            "• 點擊「未付款清單」，輸入：本週／本月／近兩個月／近三個月\n"
            "• 點擊「幫客戶下單」，輸入：客戶姓名 訂單編號 數量\n"
            "• 點擊「確認付款」，輸入：訂單編號 付款方式\n"
            "• 點擊「送達」，輸入：訂單編號"
        )
...
def _handle_customer_message(event: MessageEvent, line_bot_api: LineBotApi, user_msg: str, user_id: str, display_name: str):
    ...
    if user_id in _customer_next_input_mode and _starts_with_known_command(user_msg, _CUSTOMER_COMMAND_PREFIXES):
        previous_mode = _customer_next_input_mode.pop(user_id, None)
        ...
        try:
            if previous_mode in ("order_quantity", "simple_order"):
                ...
                LineService.push_text(line_bot_api, user_id, "已退出先前狀態")
        except Exception:
            # 推播若失敗則不影響後續指令處理
            logger.exception("failed to push exit-state notification")

    elif user_id in _customer_next_input_mode:
        mode = _customer_next_input_mode.pop(user_id)
        data = _customer_pending_data.pop(user_id, {})
        if mode == "order_quantity" or mode == "simple_order":
            parts = user_msg.strip().split()
            if len(parts) == 1 and parts[0].isdigit():
                idx = data.get("product_idx")
                quantity = int(parts[0])
                ...
                reply_text = "請輸入商品編號與數量，例：3 2，或輸入數量，例：2"
                LineService.reply_text(line_bot_api, event.reply_token, reply_text)
                return
            if quantity <= 0:
                reply_text = "數量請輸入正整數，例：2"
        ...
        elif mode == "payment_serial":
            serial_str = user_msg.strip()
            try:
                serial = int(serial_str)
            except ValueError:
                serial = None
            if serial is None or serial < 1:
                reply_text = "序號請輸入正整數，例：1"
    ...
    # ── 我的未付款 ─────────────────────────────────────────────────────
        ...
        # 如果使用者之前在等待輸入（如回報付款模式），先清除該等待狀態
        _customer_next_input_mode.pop(user_id, None)
        _customer_pending_data.pop(user_id, None)
        try:
    ...
    # ── 回報付款 <序號> ──────────────────────────────────────────────
        ...
        # 使用者可能先點了回報付款但改點其他功能，若已有等待狀態則先清除
        _customer_next_input_mode.pop(user_id, None)
        _customer_pending_data.pop(user_id, None)
    ...
    # ── 預設提示 ──────────────────────────────────────────────────────
        ...
        reply_text = (
            "歡迎光臨！您可以：\n"
            "• 點擊「查看商品」瀏覽目前商品，可點選欲購買的商品。\n"
            "• 點擊「我要下單」，並直接輸入 訂單編號 數量，例如：3 2。\n"
            "• 點擊「我的未付款」查看未付款清單。\n"
            "• 點擊「回報付款」輸入序號回報付款。\n"
            "• 點擊「聯絡老闆」立即開啟與老闆的對話。"
        )
